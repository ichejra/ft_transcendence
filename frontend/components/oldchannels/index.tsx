...
const ChatRooms = () => {
  ...
  const getSelectedChannel = (id: number) => {
    console.log("[index] get content__________________");
    ...
    dispatch(getSingleChannel(id)).then(({ payload }: any) => {
      dispatch(getChannelContent(payload.id)).then(() => {
        ...
        dispatch(setNewChannelId({ id: payload.id, render: false }));
  ...
  //! --------------------------------------------
  useEffect(() => {
    console.log("[index] NEW CHANNEL ==>==>==>==>>>>>", pathname, newChannel);
  ...
  useEffect(() => {
    ...
    if (Number(params.id) && !newChannel.render) {
      console.log("&&&&&&&&&&&&& SHOULD RENDER FIRST TIME &&&&&&&&&&");

      getSelectedChannel(Number(params.id));
    }
