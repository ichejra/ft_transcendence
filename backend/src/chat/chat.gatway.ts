...
export class ChatGatway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
    ...
    async handleChannelMessage(@ConnectedSocket() client: Socket, @MessageBody() payload: any) {
        try {
            ...
            const { channel } = await this.channelsService.getChannelById(author, payload.channelId);
            const message: MessageChannel = await this.channelsService.saveMessage(author, channel, payload.content);
